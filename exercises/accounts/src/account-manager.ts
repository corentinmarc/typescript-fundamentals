...
interface IApprovedUser extends IUser {
  isActive: boolean,
}

interface IAdmin extends IApprovedUser {
  adminSince: Date,
}
...
export class AccountManager {
   ...
   */
  activateNewUser(approver: IAdmin, userToApprove: IUser): IApprovedUser {
    if (!approver.adminSince) throw "Approver is not an admin!";
    const toConfirm = userToApprove as IApprovedUser;
   ...
   */
  promoteToAdmin(existingAdmin: IAdmin, user: IApprovedUser): IAdmin {
    ...
    const toPromote = user as IAdmin;
